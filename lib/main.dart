  ...
  Future<void> _attemptLogin() async {
    ...
    try {
      ...
      if (user == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Credenciales no v�lidas. Int�ntalo de nuevo.'),
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('No se pudo iniciar sesi�n: $error')),
      );
    } finally {
            ...
            if (isLoading)
              ...
              Wrap(
                ...
                children: teamMembers
                    .map((member) => _TeamCard(user: member))
                    .toList(),
              ),
          ],
        ),
...
class _TeamCard extends StatelessWidget {
  const _TeamCard({required this.user});
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text(
                '${user.monthlyHours} h registradas',
                style: theme.textTheme.bodyMedium?.copyWith(
                      fontWeight: FontWeight.w600,
                      color: const Color(0xFF1B1D29),
                    ),
...
List<PunchRecord> _buildInitialPunches() {
  final now = DateTime.now();
